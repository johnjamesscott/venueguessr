...
Deno.serve(async (req) => {
  try {
    ...
    const [venueRecords, competitions] = await Promise.all([
    ...
    const competition = Array.isArray(competitions) ? competitions[0] || null : null;
    const settings = getSettings(competition);
    const venues = Array.isArray(venueRecords) ? venueRecords.filter(Boolean) : [];
    ...
    const londonCities = new Set(['London', 'Rotherhithe', 'Wembley']);
    const outsideVenues = shuffled.filter((venue) => !londonCities.has(venue?.city));
    const londonVenues = shuffled.filter((venue) => londonCities.has(venue?.city));
    ...
    return Response.json({
      venues: [...primary, ...spares],
      settings,
      competitionId: competition?.id || null,
    });
  } catch (error) {
    console.error('getPlayableVenues failed:', error?.message || 'Unknown error');
